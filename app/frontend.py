...
# ==========================================
# [C1 & C2] REAL-TIME PROCESSOR (Members 1 & 2)
# ==========================================
...
def display_results(data):
    ...
    with tab1:
        if "c1_detection" in viz: st.image(base64.b64decode(viz["c1_detection"]), caption="YOLO Localization", use_container_width=True)
    with tab2:
        if "c2_hands" in viz: st.image(base64.b64decode(viz["c2_hands"]), caption="Skeleton Keypoints", use_container_width=True)
        if "angles" in res:
            c1, c2 = st.columns(2)
    ...
    with tab3:
        ...
        else: st.info("Fast Path Used - Deep Learning (C3) was skipped for efficiency.")
    with tab4:
...
if page == "📊 Analysis (Upload)":
    ...
    st.title("📹 Real-Time Analysis (C1 + C2)")
