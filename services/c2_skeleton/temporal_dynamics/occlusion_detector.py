...
is caused by:
  (A) OCCLUSION — one clock hand passing behind the other (very common)
  (B) DETECTION_LOSS — keypoint detector failed to localize a hand
  (C) REAL_TOPOLOGY_CHANGE — actual structural change (rare for clocks)

Decision Logic
--------------
1. Angular proximity: if two hands are close in angle, occlusion is likely
2. Velocity consistency: if a hand "disappears" but its last known velocity
...
class FrameState:
    """
    Snapshot of hand positions at a single frame.
...
class OcclusionDetector:
    ...
    def classify(
        ...
        if current.angle1 is not None and current.angle2 is not None:
            ...
            if angular_diff < self.occlusion_angle_threshold:
                ...
                evidence.append(
                    f"Hands are {angular_diff:.1f}° apart — within occlusion zone ({self.occlusion_angle_threshold}°)"
                )
            elif angular_diff < self.occlusion_angle_threshold * 2:
                scores[ChangeType.OCCLUSION_LIKELY] += 0.2
                evidence.append(f"Hands are {angular_diff:.1f}° apart — moderate occlusion risk")
        ...
        if velocity_check["predicted_in_frame"]:
            ...
            evidence.append(
                f"Velocity prediction: hand should be at {velocity_check['predicted_pos']} — "
        ...
        if total_checked > 0:
            ...
            if detection_rate > 0.8:
                ...
                evidence.append(f"Detection rate was only {detection_rate:.0%} — hand may genuinely be absent")

        # --- Evidence 4: Clock domain knowledge ---
        # For a real mechanical clock, hands do NOT break connections.
        # Any β₀ change is almost never a real topology change.
        scores[ChangeType.TOPOLOGY_CHANGE] *= 0.1   # Strong prior against it
        scores[ChangeType.OCCLUSION_LIKELY] += 0.15  # Clock-specific prior
        evidence.append("Domain prior: clock hands cannot physically disconnect — real topology change is rare")
    ...
    def _result(
        ...
        recommendations = {
            ChangeType.OCCLUSION_LIKELY: "Use 3D depth ordering to infer which hand is on top. Continue tracking.",
