...
=========================
Detects clock-hand skeleton connectivity using GEODESIC distances
instead of Euclidean distances on the Riemannian manifold.

Pipeline:
  1. Estimate metric tensor field g(x,y) from input image
  2. Given 2D keypoints (center, tip1, tip2), compute pairwise geodesic distances
...
Clock-specific insight:
  On a flat image, Euclidean and geodesic are the same.
  On a curved surface (car dashboard gauge, wrist watch):
    - Clock hands may appear closer in Euclidean space but far geodesically
...
class ManifoldSkeletonDetector:
    """
    Builds clock-hand graph using intrinsic (geodesic) distances.
